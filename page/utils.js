...
export function createPage(title, content, anchoCompleto = false) {
    ...
    html += "<a href=\"/\" data-nav=\"inicio\" class=\"rounded-xl px-3.5 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-800 hover:text-amber-300 md:px-4\">Inicio</a>"
    html += "<a href=\"/peliculas\" data-nav=\"peliculas\" class=\"rounded-xl px-3.5 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-800 hover:text-amber-300 md:px-4\">Películas</a>"
    html += "<a href=\"/directores\" data-nav=\"directores\" class=\"rounded-xl px-3.5 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-800 hover:text-amber-300 md:px-4\">Directores</a>"
    html += "<span class=\"mx-1 hidden h-5 w-px bg-slate-700 md:block\" aria-hidden=\"true\"></span>"
    html += "<a href=\"/peliculas/administrar\" data-nav=\"administrar\" class=\"rounded-xl border border-amber-500/35 bg-amber-500/15 px-3.5 py-2 text-sm font-semibold text-amber-400 transition hover:border-amber-400/55 hover:bg-amber-500/25 hover:text-amber-300 md:px-4\">Administrar</a>"
    html += "<span class=\"hidden bg-slate-800 text-amber-300 border-amber-400/55 bg-amber-500/25\"></span>"
    html += "</div></nav></div>"
    ...
    html += "<script>"
    html += "(function(){"
    html += "var path=location.pathname;"
    html += "var seccion=path==='/'?'inicio':path.indexOf('/peliculas/administrar')===0?'administrar':path.indexOf('/directores')===0?'directores':path.indexOf('/peliculas')===0?'peliculas':'';"
    html += "var link=document.querySelector('header nav a[data-nav=\"'+seccion+'\"]');"
    html += "if(!link)return;"
    html += "if(seccion==='administrar'){"
    html += "link.classList.add('border-amber-400/55','bg-amber-500/25','text-amber-300');"
    html += "link.classList.remove('border-amber-500/35','bg-amber-500/15','text-amber-400');"
    html += "}else{"
    html += "link.classList.add('bg-slate-800','text-amber-300');"
    html += "link.classList.remove('text-slate-300');"
    html += "}"
    html += "})();"
