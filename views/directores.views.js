...
export function listaDirectores(directores) {
    ...
    if (!directores.length) {
        html += "<p class=\"rounded-xl border border-slate-800 bg-slate-900 p-6 text-slate-400\">Todavía no hay directores cargados.</p>"
...
export function detalleDirector(director) {
    let html = ""
...
export function formularioNuevoDirector() {
    ...
    html += "<input name=\"name\" required class=\"w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500\">"
    html += "</div>"
    html += "<div>"
    html += "<label class=\"mb-2 block text-sm text-slate-300\">URL de la foto</label>"
    html += "<input name=\"photo\" type=\"url\" required class=\"w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500\">"
    html += "</div>"
    html += "<div>"
