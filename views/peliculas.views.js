...
export function listaPeliculas(peliculas, filtros = {}, anios = []) {
    ...
    if (!peliculas.length) {
        html += "<p class=\"rounded-xl border border-slate-800 bg-slate-900 p-6 text-slate-400\">No hay películas disponibles con esos filtros.</p>"
...
export function detallePelicula(pelicula) {
    let html = ""
...
export function formularioNuevaPelicula() {
    let html = ""
    ...
    html += "<input name=\"title\" required class=\"w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500\">"
    html += "</div>"
    html += "<div>"
    ...
    html += "<input name=\"image\" type=\"url\" required class=\"w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500\">"
    html += "</div>"
    html += "<div>"
    html += "<label class=\"mb-2 block text-sm text-slate-300\">Año</label>"
    html += "<input name=\"year\" type=\"number\" required class=\"w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500\">"
    html += "</div>"
    html += "<div>"
    html += "<label class=\"mb-2 block text-sm text-slate-300\">Género</label>"
    ...
    html += "<input name=\"trailer\" type=\"url\" class=\"w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500\">"
    html += "</div>"
...
export function formularioEditarPelicula(pelicula) {
    let html = ""
    html += "<div class=\"mx-auto max-w-2xl\">"
    ...
    html += "<div>"
    html += "<label class=\"mb-2 block text-sm text-slate-300\">URL del tráiler</label>"
    html += `<input name="trailer" type="url" value="${pelicula.trailer || ""}" class="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-50 outline-none focus:border-amber-500">`
    html += "</div>"
...
export function pagina404() {
    return createPage("No encontrada", "<p class=\"text-slate-400\">Página no encontrada.</p>")
}
